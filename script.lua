...
-- Toggle, Slider, Button Functions
...
local function updateESP()
    ...
    clearESP() -- Clear all old ESP elements to prevent performance issues
    ...
    for _, p in pairs(cachedPlayers) do
        if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") and p.Character:FindFirstChild("Humanoid") then
            ...
            if targetHead then
                ...
                local ng = create("BillboardGui", {Size = UDim2.new(0, 100, 0, 50), StudsOffset = Vector3.new(0, 3, 0), Adornee = targetHead, AlwaysOnTop = true, Parent = p.Character})
                ...
                if espBoxActive then
                    ...
                    b.Transparency = 0.7
                    b.AlwaysOnTop = true
                    b.Adornee = targetRoot
                    b.Parent = p.Character
                    espBoxes[p] = b
                end

                if espTracerActive and rootPart then
                    ...
                    t.Attachment1 = create("Attachment", {Parent = targetRoot})
                    t.Parent = rootPart
                    espTracers[p] = t
                end
...
local ef, et = addToggle("ESP", false, 120, function(on) espActive = on if not on then clearESP() else updateESP() end end, visualsTab)
...
local csi = create("TextBox", {Size = UDim2.new(0.15, 0, 0, 20), Position = UDim2.new(0.6, 0, 0.5, -10), BackgroundColor3 = colors.Button, Text = tostring(cframeSpeedValue), TextColor3 = colors.Text, TextSize = 12, Font = Enum.Font.FredokaOne, Parent = csf, ClearTextOnFocus = false})
...
csi.FocusLost:Connect(function(e)
    if e then
        ...
        csi.Text = tostring(cframeSpeedValue)
    end
end)
...
local fsi = create("TextBox", {Size = UDim2.new(0.15, 0, 0, 20), Position = UDim2.new(0.6, 0, 0.5, -10), BackgroundColor3 = colors.Button, Text = tostring(flySpeedValue), TextColor3 = colors.Text, TextSize = 12, Font = Enum.Font.FredokaOne, Parent = ff, ClearTextOnFocus = false})
...
local fcsi = create("TextBox", {Size = UDim2.new(0.15, 0, 0, 20), Position = UDim2.new(0.6, 0, 0.5, -10), BackgroundColor3 = colors.Button, Text = tostring(freeCamSpeed), TextColor3 = colors.Text, TextSize = 12, Font = Enum.Font.FredokaOne, Parent = fcf, ClearTextOnFocus = false})
...
fcsi.FocusLost:Connect(function(e)
    if e then
        ...
        fcsi.Text = tostring(freeCamSpeed)
    end
end)
...
local targetInput = create("TextBox", {Size = UDim2.new(0.7, 0, 1, 0), Position = UDim2.new(0, 5, 0, 0), BackgroundColor3 = colors.Button, Text = "", TextColor3 = colors.Text, TextSize = 14, Font = Enum.Font.FredokaOne, PlaceholderText = "Enter player name", Parent = targetInputFrame, ClearTextOnFocus = false})
...
-- "Load to Custom" Button moved to y=330
...
local function addAnimStageRow(stage, y)
    ...
    local idInput = create("TextBox", {Size = UDim2.new(0, 100, 0, 25), Position = UDim2.new(0, 105, 0, 2), BackgroundColor3 = colors.Button, Text = animationStages[stage].id, PlaceholderText = "ID", TextColor3 = colors.Text, TextSize = 12, Font = Enum.Font.FredokaOne, Parent = stageFrame, ClearTextOnFocus = false})

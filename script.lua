...
local lastSelectedPreset = nil -- To track the last clicked standard animation
...
addToggle("Infinite Zoom", false, 160, function(on) player.CameraMaxZoomDistance = on and 10000 or originalSettings.MaxZoom end, visualsTab)
...
csf, cst = addToggle("CFrame Speed", false, 0, function(on)
	...
	if on and flyActive then flyToggle(false) end -- Deactivate Fly if Speed is activated
end, playerTab)
local csi = create("TextBox", {Size = UDim2.new(0.15, 0, 0, 20), Position = UDim2.new(0.6, 0, 0.5, -10), BackgroundColor3 = colors.Button, Text = tostring(cframeSpeedValue), TextColor3 = colors.Text, TextSize = 12, Font = Enum.Font.FredokaOne, Parent = csf})
...
ff, flyToggle = addToggle("Fly", false, 40, function(on)
	...
	flyActive = on
	if on and cframeSpeedActive then cst(false) end -- Deactivate Speed if Fly is activated
...
local targetInput = create("TextBox", {Size = UDim2.new(0.7, 0, 1, 0), Position = UDim2.new(0, 5, 0, 0), BackgroundColor3 = colors.Button, Text = "", TextColor3 = colors.Text, TextSize = 14, Font = Enum.Font.FredokaOne, PlaceholderText = "Enter player name", Parent = targetInputFrame})
...
loadPresetBtn.MouseButton1Click:Connect(function()
	if lastSelectedPreset then
		...
		animationStages.swim.id = idle1 -- Use idle1 for swim as default
...
local function addAnimStageRow(stage, y)
	...
	local idInput = create("TextBox", {Size = UDim2.new(0, 100, 0, 25), Position = UDim2.new(0, 105, 0, 2), BackgroundColor3 = colors.Button, Text = animationStages[stage].id, PlaceholderText = "ID", TextColor3 = colors.Text, TextSize = 12, Font = Enum.Font.FredokaOne, Parent = stageFrame})
...
for _, anim in pairs(animButtons) do
	...
	create("UICorner", {CornerRadius = UDim.new(0, 8), Parent = b})
	b.MouseButton1Click:Connect(function()
		lastSelectedPreset = anim -- Store the last selected preset
...
UserInputService.InputBegan:Connect(function(i, gp)
	...
	elseif keybinds.Fly and isBindPressed(keybinds.Fly, i) then
		...
		if flyActive then cst(false) end -- Ensure Speed is off when Fly is toggled on
	elseif keybinds.CFrameSpeed and isBindPressed(keybinds.CFrameSpeed, i) then
		cst(not cframeSpeedActive)
		if cframeSpeedActive then flyToggle(false) end -- Ensure Fly is off when Speed is toggled on
